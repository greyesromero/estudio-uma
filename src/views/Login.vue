...
<div>
	...
	<v-content class="mb-10">
		<section>
			<div class="py-12"></div>
			<div class="py-12 d-none d-sm-flex"></div>
			
			<v-container fluid>

				<v-row justify="center">
					<v-col cols="12" md="4" xs="12">
						<v-card :loading="loading" class="mx-auto" dark min-width="345" color="transparent" tile flat>
							
							<v-card-text>
								<h1 class="secondary--text text-center mb-5">Iniciar Sesión</h1>

								<v-form v-model="valid" :lazy-validation="lazy"  ref="form">
									<v-text-field v-model.lazy="email" outlined color="secondary" label="Correo Electrónico"  :rules="emailRules" required></v-text-field>
									<v-text-field type="password" v-if="visible" v-model.lazy="token" outlined color="secondary" label="Token" :rules="tokenRules"></v-text-field>
							...
							<v-card-actions class="align-self-center align-content-center justify-center center align-center">
								...
								<v-btn v-if="token_button" tile color="accent" @click="token_validation" class="black--text font-weight-bold text-none pl-5 pr-5" dark >Generar Token</v-btn>
								<v-btn v-if="login_button" tile color="accent" @click="login" class="black--text font-weight-bold text-none pl-5 pr-5">Iniciar Sesión</v-btn>
